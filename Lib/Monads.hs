...
{- A collection of common monads and monad transformers used in Haskell including:
 -   - State / StateT monad                              [discussed in class]
 -   - Except / ExceptT (Either monad transformer) monad [not covered in class]
 -   - Reader / ReaderT monad                            [not covered in class]
 -   - Identity monad                                    [not covered in class, but should be straightforward]
 -
 -   - Alternative instances for monads                  [not covered in class]
 -
 -
 - Further optional reading:
 -  - This link is the (probably) most commonly used Haskell library for monad
 -  transformers.  The code presented should like quite similar to what is
 -  presented in that library (function names should be the same,
 -  implementations may be syntactically different, but should be identical to
 -  use): https://hackage.haskell.org/package/transformers
 -
 -  - A very comprehensive document on type checking, type classes, monads,
 -  monad transformers.  However, this paper is quite old, and people name the
 -  functions differently nowadays, so it may be a bit confusing with the
